...
function renderConfig() {
  ...
  // Solo re-renderiza el panel del juego si cambia la dificultad,
  // no en cada keystroke. Lo forzamos siempre pero el panel interno
  // usa window.STATE.customWords que persiste.
...
function setConfig(key, val) {
  ...
  if (key === 'difficulty') {
    // Solo en cambio de dificultad re-renderiza el panel completo
    renderConfig();
  } else {
    // Para wordCount/timeLimit: actualiza solo esos botones sin tocar el panel del juego
    if (key === 'wordCount') {
      document.querySelectorAll('#wordCountOpts .opt-btn').forEach(b => {
        b.classList.toggle('selected', parseInt(b.textContent) === val);
      });
    }
    if (key === 'timeLimit') {
      document.querySelectorAll('#timeOpts .opt-btn').forEach(b => {
...
function setConfig(key, val) {
  ...
  // Si solo cambia difficulty y ya hay un panel custom activo, re-render completo
  // para mostrar/ocultar el panel. Para otros keys, solo actualiza botones/selección.
  if (key === 'difficulty') {
    renderConfig();
  } else {
    // Actualiza solo los botones de selección sin destruir el panel del juego
    document.querySelectorAll('#wordCountOpts .opt-btn').forEach(b =>
      b.classList.toggle('selected', b.textContent.trim().startsWith(String(STATE.config.wordCount)))
    );
    document.querySelectorAll('#timeOpts .opt-btn').forEach(b =>
      b.classList.toggle('selected', b.getAttribute('onclick').includes(`'${val}'`) || b.getAttribute('onclick').includes(`,${val})`))
    );
    updateConfigBtn();
  }
}

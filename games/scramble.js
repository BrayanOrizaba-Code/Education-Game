...
(function () {
  ...
  const round = {
    words: [], wordIndex: 0, startTime: null, timerInterval: null,
    answer: [], scrambled: [], blocked: false, _paused: false,
    _pausedElapsed: 0, _onNextPlayer: null, _onGameOver: null,
    _appState: null, _navFn: null,
  };

  // ── Solo actualiza el botón Continue sin re-renderizar el panel ──
  function _updateConfigBtnOnly() {
    ...
    if (!btn) return;
    const ok        = appState.config.difficulty !== 'custom' || n >= 3;
    btn.disabled    = !ok;
    btn.textContent = ok ? 'Continue →' : `Add at least 3 words (${n}/3)`;
  }
  ...
  function renderConfigPanel(container, config) {
    if (config.difficulty !== 'custom') { container.innerHTML = ''; return; }
  ...
  function validateConfig(config, appState) {
    if (config.difficulty === 'custom') {
      const words = (appState && Array.isArray(appState.customWords))
        ? appState.customWords
        : (window.STATE.customWords || []);
  ...
  function _buildWordPool(config, appState) {
    if (config.difficulty === 'custom') {
      const words = (appState && Array.isArray(appState.customWords) && appState.customWords.length)
        ? appState.customWords
        : (window.STATE.customWords || []);
  ...
  function _renderTiles() {
    ...
    document.getElementById('checkBtn').disabled =
      round.answer.filter(c => c !== null).length < wordLen;
  }
  ...
  function _pauseGame() {
    round._pausedElapsed = (Date.now() - round.startTime) / 1000;
    round._paused = true;
    openPause(
      function () {
  ...
  // ════════════════════════════════════════
  const ScrambleGame = {
    id: 'scramble', name: 'Word Scramble', icon: '🔤',
    description: 'Unscramble the letters to form the correct English word.',
    difficulty: 3, available: true,
    renderConfigPanel, validateConfig, startRound,
    _placeLetter, _removeFromSlot, _clearAnswer, _checkAnswer,
    _addCustomWord, _removeCustomWord,
  };

...
// ══════════════════════════════════════════
//  READING COMPREHENSION  v1.1  (games/reading.js)
...
(function () {
  ...
  function renderConfigPanel(container) {
    ...
    // Always force difficulty to 'custom' for this game
    appState.config.difficulty = 'custom';
  ...
  function startRound(appState, navFn, onNextPlayer, onGameOver) {
    ...
    // Navigate first, then populate the screen on the next tick
    // so the DOM elements are visible and writable
    navFn('reading-game');
  ...
  function _showReadingScreen() {
    ...
    // Reset both views to known states
    ...
    readingView.style.flexDirection = 'column';
    readingView.style.minHeight = '100vh';
  ...
  function _pauseReading() {
    openPause(
      () => {},         // onResume: nothing to do (no timer)
      null              // onQuit: handled by app.js
    );
  }
  ...
  // Expose on window BEFORE registering so onclick="" works in config panel
